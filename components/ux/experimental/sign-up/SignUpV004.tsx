...
export function SignUpV004() {
  ...
  const cardHeight = useMemo(() => {
    if (hasEmail) return 495;
    if (emailMode) return 445;
    return 315;
  }, [emailMode, hasEmail]);
  ...
  return (
    <motion.main
      className="relative flex min-h-screen items-center justify-center bg-white px-6 py-6 font-['Montserrat',ui-sans-serif,system-ui,sans-serif] text-black"
      ...
      <section className="flex w-full max-w-[448px] flex-col items-center">
        ...
        >
          <div className="px-8 pt-8">
            ...
            >
              <button
                className="flex h-14 w-full items-center justify-center gap-3 rounded-[12px] bg-white text-[16px] font-medium leading-6 text-[#101828] transition hover:bg-[#fffaf7] disabled:cursor-not-allowed disabled:opacity-75"
          ...
          <motion.div
            className="absolute bottom-0 left-0 w-full p-[5px]"
            layout
            transition={spring}
          >
            <div className="rounded-[12px] border-t border-[#e5e7eb] bg-white px-[27px] pb-[13.5px] pt-5">
...
function BrandMark() {
  return (
    <div className="absolute left-6 top-6 flex items-center gap-2">
      ...
      >
        <span className="text-[14px] font-semibold leading-5 text-white">T</span>
      </div>
      <p className="text-[18px] font-semibold leading-6 text-black">Tavyn</p>
    </div>
  );
}
